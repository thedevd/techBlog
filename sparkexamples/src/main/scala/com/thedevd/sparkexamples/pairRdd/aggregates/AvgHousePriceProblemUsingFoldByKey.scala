...
*/
object AvgHousePriceProblemUsingFoldByKey {

  def main(args: Array[String]): Unit = {
    Logger.getLogger("org").setLevel(Level.OFF)

    val spark = SparkSession.builder()
      .appName("AvgHousePriceProblemUsingFoldByKey")
      .master("local[*]")
      .getOrCreate()
    ...
    // Counting the no of rows and totalHousePrice using foldByKey
    /*
     * foldByKey() is similar to reduceByKey() except it involves a initial value or zeroValue.
     * In foldByKey() and reduceByKey() the input and output type need to be same,
     * where as in aggregateByKey() and combineByKey(), they can be of different type.
     */
    val zeroValue= (0, 0.0)
    val bedroomToCountToTotalPricePairRdd = housePricePairRdd
      .foldByKey(zeroValue)((accumulated: (Int, Double), value: (Int, Double)) => {
        (accumulated._1 + value._1, accumulated._2 + value._2) //  totalCount, totalPrice
      })
