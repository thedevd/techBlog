...
object CallByValueVsCallByName {
  ...
  def main(args: Array[String]): Unit = {
     ...
     * 1. At runtime the value to callMeByValue() method is evaluated before the calling the method, so same value is printed
     ...
     *
     *    Interestingly this is called many places in scala library for example in Try(), and that is the reason we can write the Try() statement
     *    as Try(throw new NullPointerException()). And if Try's apply() method was defined as call be value then we would never been able to write
